...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        qualtrics_user_univ_ids = list(QualtricsUser.objects.filter(last_login__gte='2018-09-23 12:00:00+00',
                                                                    division__isnull=False,
                                                                    user_type__isnull=False).values_list('univ_id', flat=True))
        ...
        # chunked_list = [qualtrics_user_univ_ids[i:i + 500] for i in xrange(0, len(qualtrics_user_univ_ids), 500)]
        ...
        for q_id in qualtrics_user_univ_ids:
            try:
                # print len(chunk)
                # print 'Getting IAM people data'
                # person_iam_queryset = Person.objects.filter(pk__in=chunk, prime_of_primes_flag='Y')
                try:
                ...
                # for person_iam in person_iam_queryset:
                if person_iam:
                    ...
                    # # Update the QualtricsUser object to reflect the current LPP data
                    # # This should be commented out on the second run when you switch data sources
                    # person.division = person_iam.division
                    # person.user_type = person_iam.role
                    # person.save()
